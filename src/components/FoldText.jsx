...
const FoldText = ({
  ...
  const segments = useMemo(() => {
    ...
    // When splitBy === 'char', group characters into word spans to prevent breaking inside words
    return text.split(/(\s+)/).flatMap((part, index) => {
      if (!part) return [];
      if (/^\s+$/.test(part)) return renderWhitespace(part, `ws-${index}`);
      return (
        <span className="fold-text-word" key={`word-wrap-${index}`}>
          {Array.from(part).map((char, cIdx) =>
            renderSegment(char, `segment-char-${segmentIndex}-${cIdx}`)
          )}
        </span>
      );
    });
